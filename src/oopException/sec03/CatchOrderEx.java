...
public class CatchOrderEx {
	...
	public static void main(String[] args) {
		...
		// 모든 예외처리 클래스는 Exception 클래스를 상속받아서 구현되어있음
		...
		} /*catch(Exception e) { // 모든 예외 처리하는 catch 구문이므로 아래 표현된 catch는 필요없음 - 문법 오류
			System.out.println("오류발생 : 상위 클래스");
		}*/ catch(NumberFormatException e) {
			System.out.println("오류발생 : 변환오류");
		} catch(Exception e) {
			// 특정 오류 외의 모든 오류 처리하게 하려고 하면 현재 catch 구문은 순서를 가장 아래로 내려야함
			System.out.println("오류발생 : 상위 클래스");
		} finally {
